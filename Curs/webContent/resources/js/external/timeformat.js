!function () {
    ...
    function s(s, n) {
        function r(t, i) {
            var e = h.offset(), s = /^touch/.test(t.type), o = e.left + f, c = e.top + f,
                r = (s ? t.originalEvent.touches[0] : t).pageX - o, l = (s ? t.originalEvent.touches[0] : t).pageY - c,
                u = Math.sqrt(r * r + l * l), m = !1;
            if (!i || !(u < b - w || u > b + w)) {
                t.preventDefault();
        ...
        var l = c(M), h = l.find(".clockpicker-plate"), m = l.find(".picker__holder"), v = l.find(".clockpicker-hours"),
            P = l.find(".clockpicker-minutes"), C = l.find(".clockpicker-am-pm-block"),
            x = "INPUT" === s.prop("tagName"), T = x ? s : s.find("input"), _ = c("label[for=" + T.attr("id") + "]"),
            V = this;
        ...
        if (n.twelvehour)for (S = 1; S < 13; S += 1)B = I.clone(), D = S / 6 * Math.PI, E = b, B.css("font-size", "140%"), B.css({
            left: f + Math.sin(D) * E - w,
            top: f - Math.cos(D) * E - w
        }), B.html(0 === S ? "00" : S), v.append(B), B.on(u, r); else for (S = 0; S < 24; S += 1) {
        ...
        if (h.on(u, function (t) {
            ...
            var z = l.find(".clockpicker-canvas"), U = t("svg");
    ...
    var c = window.jQuery, n = c(window), a = c(document), r = "http://www.w3.org/2000/svg",
        ...
        m = navigator.vibrate ? "vibrate" : navigator.webkitVibrate ? "webkitVibrate" : null, v = 0, f = 135, b = 110,
        g = 80, w = 20, y = 2 * f, A = l ? 350 : 1,
    ...
    s.DEFAULTS = {
        ...
        var s = "hours" === t, n = s ? this.hoursView : this.minutesView, a = s ? this.minutesView : this.hoursView;
        ...
        var i = this.currentView, e = this[i], s = "hours" === i, o = Math.PI / (s ? 6 : 30), c = e * o,
            n = s && e > 0 && e < 13 ? g : b, a = Math.sin(c) * n, r = -Math.cos(c) * n, l = this;
        ...
        var n, a = Math.atan2(t, -e), r = "hours" === this.currentView, l = Math.PI / (r || s ? 6 : 30),
            h = Math.sqrt(t * t + e * e), u = this.options, d = r && h < (b + g) / 2, k = d ? g : b;
        ...
        var v = Math.sin(a) * (k - w), f = -Math.cos(a) * (k - w), y = Math.sin(a) * k, A = -Math.cos(a) * k;
